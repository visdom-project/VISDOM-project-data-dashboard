...
export const groupByDate = async (dataToGroup, dateReference) => {
  ...
  for (const month of monthArray) {
    const monthData = dataToGroup.filter(
      (groupData) =>
        parseInt(moment(groupData.data[dateReference]).format("M")) === month
    );
...
export const cumulativeGitCommitData = (groupCommitData) => {
  ...
  gitStats.map((commitData) => {
    ...
    const gitCommitObj = {
      index: parseInt(key),
      averageFileChanged:
        totalCommits == 0 ? 0 : Math.round(cumFiles / totalCommits),
      averageAdditions:
        totalCommits == 0 ? 0 : Math.round(cumAdditions / totalCommits),
      averageDeletions:
        totalCommits == 0 ? 0 : Math.round(cumDeletions / totalCommits),
...
export const latestSonarQubeData = (groupedSonarQubeData) => {
  ...
  sonarQubeData.map((data) => {
    ...
    const latestDataOfMonth = value.sort((a, b) =>
      a.last_commit_date.localeCompare(b.last_commit_date)
    );
...
export const cumulativeJiraData = (groupedJiraIssueData) => {
  ...
  jiraData.map((data) => {
    ...
    const monthObj = {
      ...
      averageTimePerIssue:
        totalIssues !== 0 ? Math.round(totalResolutionHours / totalIssues) : 0,
    };
